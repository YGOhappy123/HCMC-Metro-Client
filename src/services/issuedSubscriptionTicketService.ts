...
export type IssuedSubscriptionTicketSortAndFilterParams = {
...
const issuedSingleJourneyTicketService = ({ enableFetching }: { enableFetching: boolean }) => {
    ...
    const [tickets, setTickets] = useState<IIssuedSubscriptionTicket[]>([])
    ...
    const buildQuery = ({
        ...
        sort,
        range
    }: IssuedSubscriptionTicketSortAndFilterParams) => {
        const query: any = {}
    ...
    const searchTicketsQuery = useQuery({
        queryKey: ['search-issued-subscription-tickets', query, sort],
        queryFn: () => {
            return axios.get<IResponseData<IIssuedSubscriptionTicket[]>>(
                `/issued-tickets/subscription?skip=${limit * (page - 1)}&limit=${limit}&filter=${query}&sort=${sort}`
            )
        },
        enabled: false
    })
