...
const pathService = ({ enableFetching }: { enableFetching: boolean }) => {
    ...
    const getAllStaffsQuery = useQuery({
        ...
        queryFn: () => {
            ...
            return axios.get<IResponseData<IStaff[]>>(`/stations/metro-path-enriched?start=${limit * (page - 1)}&end=${limit}&method=${}`)
        },
