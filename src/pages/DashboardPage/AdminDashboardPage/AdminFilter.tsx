...
const AdminFilter = ({ setHavingFilters, onChange, onSearch, onReset }: AdminFilterProps) => {
  ...
  useEffect(() => {
    console.log('Filter params:', { searchName, searchEmail, searchPhoneNumber, sort, range });
    onChange({ searchName, searchEmail, searchPhoneNumber, sort, range });
  }, [searchName, searchEmail, searchPhoneNumber, sort, range]);

  const handleSearch = () => {
    console.log('Search triggered:', { searchName, searchEmail, searchPhoneNumber, sort, range });
    onSearch();
    ...
    <PopoverContent className="w-[400px] bg-white">
      <div className="mb-4 flex items-center justify-between">
        <Button
          text="Tìm kiếm"
          variant="gradient"
          className="border-primary rounded-2xl px-3 py-1.5 text-xs"
