...
type StationsChoosingFormProps = {
    ...
    setEntry: (value: IStation | null) => void
    setExit: (value: IStation | null) => void
    setMethod: (value: PaymentMethod) => void
}

const StationsChoosingForm = ({ setPath, setEntry, setExit, setMethod }: StationsChoosingFormProps) => {
    ...
    const handleSearch = async () => {
        ...
        setEntry(stations.find(st => st.stationId === Number(entryStation)) || null)
        setExit(stations.find(st => st.stationId === Number(exitStation)) || null)
        setMethod(paymentMethod)
    }

    return (
        <div className="flex h-[98px] w-full gap-10 rounded-lg border-2 border-solid border-[#9C9C9C] bg-white p-5 text-[#4D4D4D]">
